...
static void swap_endian_ui(unsigned int* v) {
    *v = (*v >> 24) | ((*v << 8) & 0x00FF0000) | ((*v >> 8) & 0x0000FF00) |
        (*v << 24);
}
...
ImageBMP* ImageBMP_Create(char* fileName) {
    ImageBMP* ret;
    ret = malloc(sizeof(ImageBMP));
    // try to initialize it and if it fails return 0
    if (ImageBMP_Init(ret, fileName) == false) {
        return 0;
    }
    return ret;
...
char ImageBMP_Init(ImageBMP* bmp, char* fileName) {
    ...
    // 0x00 From the BMP HEADER: Read the signature of the file and if it is not
    // the BMP file magic number stop
    if (bmpH.magicNumber[0] != 0x42)
        return false;

    // 0x02 From the BMP HEADER: Read the size of the file
    bmp->fileSize = bmpH.fileSize;
    if (is_big_endian())  //.bmp is always litle endian so if the system is big
                          //endian make sure to change it
    {
                          ...
                          //endian make sure to change it
    {
        ...
        swap_endian_ui(
            (unsigned int*)&bmpIH.imgWidth);  // casting them since we only do
    ...
    // If size is not 40, then it must be a different version of the info
    // header. It's not supported so return
    if (bmpIH.headerSize != 40)
        return false;

    // no compression method is supported yet, so make sure that it is
    // uncompressed
    ...
    // if bits per pixel is less than or equal to 8 we do have a color table so
    // read that in too
    if (bmpIH.bpp <= 8) {
        // The number of colours which is 2^n  , where n is the bits per pixel
        // of the image
    ...
    // Calculate the bytes required to store a row of the image (padding
    // included)
    unsigned int imgWidthBytes =
        ceil((float)(bmp->imgWidth * (float)bmpIH.bpp / 32)) * 4;
    // the img width in bytes without padding
    unsigned int imgWidthBytesNP =
        ceil((float)bmp->imgWidth * (float)bmpIH.bpp / 8);
    // calculate the offset (the difference between the normal imgWidth and the
    // one with padding
    int offset = imgWidthBytes - imgWidthBytesNP;
    // since some times the raw size inside the info header can be zero we
    // obtain the raw size from this calculation
    unsigned int dataSize = bmp->imgHeight * imgWidthBytes;
    // a buffer to hold the data of the bitmap file we are going to read, we
    // don't read it from the header since it might be zero some times there
    ...
    // Begin reading image data from the point of the file where they should be
    // placed
    ...
    if (bmp->imgHeight > 0) {
        ...
        case 24:
            ...
            bmp->type = _24BIT_BMP;
            // i is counter for bytes in the image, j is counter for bytes
            // in the bitmap file
        ...
        case 4:
            ...
            for (i = 0, j = 0; j < dataSize - 3; i += 6, j++) {
                ...
                if ((j + choffset) % imgWidthBytes == 0) {
                    ...
                    // if the last row pixel uses half the byte, backtrack a
                    // bit
                    if (bmp->imgWidth % 2 != 0)
                        i -= 3;
                    // if the addition of offset throws us off the dataSize
                    // (which is the for condition)
                    if (j >= dataSize)
                        break;  // stop
                }
            }
        ...
        case 1: {
            ...
            for (i = 0; i < dataSize - 3; i++) {
                for (j = 7; j >= 0; j--)  // for each bit of that byte
                {
                    rowBitIndex++;
                    if (rowBitIndex >
                        bmp->imgWidth)  // if we are at the end of a row
                    {
                        i += offset;
                        rowBitIndex = 0;
                        break;
                    ...
                    bmp->imgData[byteIndex] |=
                        (((dataBuff[i] >> j) & 0x1) << bitIndex);
...
int ImageBMP_Save(ImageBMP* bmp, char* fileName) {
    ...
    // since this is just for testing, only one type of bitmap (24 bpp)will be
    // saved
    ...
    // a buffer to hold the data of the bitmap file we are going to write, we
    // don't read it from the header since it might be zero some times there
    unsigned char* dataBuff;
    dataBuff = malloc(dataSize);
    // i is counter for bytes in the image, j is counter for bytes in the bitmap
    // file
    for (i = 0, j = 0; j < dataSize - 3; i += 3, j += 3) {
        // jump over the padding at the start of a new line
        if ((j + offset) % imgWidthBytes == 0) {
